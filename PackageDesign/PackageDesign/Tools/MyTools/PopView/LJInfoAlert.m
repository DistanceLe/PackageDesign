...
static LJInfoAlert* toast;
+(void)showToastInfo:(NSString *)info{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        toast = [[LJInfoAlert alloc]initWithFrame:CGRectMake(0, 0, 0, 0)];
        if (!toast.toastHud) {
            toast.toastHud = [[MBProgressHUD alloc]initWithFrame:CGRectMake(0, 0, IPHONE_WIDTH, IPHONE_HEIGHT)];
            [[kDataManager getRootWindws] addSubview:toast.toastHud];
        }
        toast.toastHud.mode = MBProgressHUDModeText;
        toast.toastHud.yOffset = 70;
        toast.toastHud.userInteractionEnabled = NO;
    });
    ...
    [toast.toastHud.superview bringSubviewToFront:toast.toastHud];
    
    
    [toast.toastHud setLabelText:info];
    [toast.toastHud show:YES];
    CGFloat showTime = 2;
    if (info.length > 5) {
        showTime = showTime + (info.length-5)/5*0.2;
    }
    [toast.toastHud hide:YES afterDelay:showTime];
    return;
}
...
//    height = height < 0 ? 0 : height > 1 ? 1 : height;
    ...
    __block UIWindow* mainWindow=[kDataManager getRootWindws];
...
//    NSArray* windows=[kDataManager getRootWindws];
