...
static const CGFloat SVProgressHUDVerticalSpacing = 26.0;
static const CGFloat SVProgressHUDHorizontalSpacing = 26.0;
...
+ (SVProgressHUD*)sharedView {
    ...
    dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame:[kDataManager getRootWindws].bounds]; });
#else
...
+ (void)setStatusColor:(UIColor*)color{
    [self sharedView].statusLabel.textColor = color;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        _minimumSize = CGSizeMake(115, 128);
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    self.frame = [kDataManager getRootWindws].bounds;
...
#if !defined(SV_APP_EXTENSIONS) && TARGET_OS_IOS
                    UIViewController *rootController = [kDataManager getRootWindws].rootViewController;
...
- (UIControl*)controlView {
    ...
    // Update frames
#if !defined(SV_APP_EXTENSIONS)
    CGRect windowBounds = [kDataManager getRootWindws].bounds;
